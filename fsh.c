...
void execute(struct parsed_line *p)
{
    ...
    /* PART 10: Implementing Connectives */
...
void commander(struct parsed_line *p)
{
    ...
    /* stores complete path names */
    /* buf1 for command 1 */
    /* buf2 for command 2 */
    char buf1[1000] = {0};
    char buf2[1000] = {0};

    /* print '$' again if no commands given */
    if (p->pl == NULL) {
        pass = 1;
        laststatus = 0;
    } else {
        invalid = 0;
        /* find PATH for first command */
        find(buf1, p->pl);
        /* find PATH for second command (if given) */
        if (p->pl->next != NULL)
            find(buf2, p->pl->next);
    }
    ...
    while (pass == 0 && invalid != 1) {
        ...
        switch ((x = fork())) {
            ...
            /* PART 5: Implementing I/O Redirection */

                /* open outfile if given in command */
                if (p->outputfile) {
                    close(out);
                ...
                /* PART 6: if pipe valid, execute the following */
                if (p->pl->next != NULL) {
                    int pipefd[2];
...
/* PART 4: Find path of command in directories */
...
/* PART 3: execute command eith execve*/
